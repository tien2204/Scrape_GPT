...
FAL_EVENT_FIELDS = ["request_id", "endpoint_id", "cost_total", "timestamp"]
...
def _read_existing_request_ids(csv_path: str) -> set:
    if not os.path.exists(csv_path):
        return set()
    with open(csv_path, "r", newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        return {row["request_id"] for row in reader}


def merge_fal_event_rows(csv_path: str, new_rows: list[dict]) -> int:
    existing = _read_existing_request_ids(csv_path)
    to_add = [row for row in new_rows if row["request_id"] not in existing]
    file_exists = os.path.exists(csv_path)
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=FAL_EVENT_FIELDS)
        if not file_exists:
            writer.writeheader()
        for row in to_add:
            writer.writerow(row)
...
def get_new_fal_event_rows(csv_path: str, candidate_rows: list[dict]) -> list[dict]:
    existing = _read_existing_request_ids(csv_path)
    return [row for row in candidate_rows if row["request_id"] not in existing]
